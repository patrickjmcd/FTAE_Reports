...
{
    //[super viewDidLoad];
    NSString *rubyServer = [[NSUserDefaults standardUserDefaults] stringForKey:@"rubyServer"];
    NSString *JSONurlString = [NSString stringWithFormat: @"%@%@%@",  @"http://",rubyServer, @":3000/override.json"];
    ...
    NSURL *url = [[NSURL alloc] initWithString:JSONurlString];
    ...
    AFJSONRequestOperation *operation = [AFJSONRequestOperation JSONRequestOperationWithRequest:request success:^(NSURLRequest *request, NSHTTPURLResponse *response, id JSON) {
        ...
        NSString *errorMessage = [NSString stringWithFormat:@"%@%@", @"Could not retrieve JSON from ", JSONurlString ];
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"JSON Error" message:errorMessage delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
        [alert show];
    }];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    alarmDetailViewController.conditionName = self.overrideAlarms[indexPath.row][@"inAlarm"];
